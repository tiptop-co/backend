...
- Два сервера под `DEV` и `STAGE` окружения соответственно на Ubuntu 22-24 Server
- Развертывание приложений на серверах в Docker контейнерах
- Для пользователя необходим браузер на базе `Chromium`

## Правила выгрузки

- При успешном прохождени пайплайна в рабочей ветке приложение автоматически развертывается в `STAGE` окружении
